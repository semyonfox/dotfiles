...
## Authenticated A/B / PR Comparisons

When comparing a PR against a base branch for an authenticated app page, prefer a reproducible two-worktree setup over ad-hoc browser impressions:

- Run both variants from production builds (`next build`/`next start` or equivalent), not dev servers.
- Use a disposable seeded test user and deterministic fixture content that exercises the feature under test.
- Verify Lighthouse is auditing the authenticated target URL, not a silent redirect to `/login`.
- Combine page-load Lighthouse metrics with a Playwright interaction probe for lazy-loaded UI. Median-of-5 timings are usually enough for local comparisons.
- Capture individual screenshots plus a labelled side-by-side composite so the user can judge the visual tradeoff directly.
- When the visual difference is subtle, also make zoomed landscape crops around the exact UI under judgment; tall full-page composites are often unreadable in chat previews.
- Inspect the screenshots yourself before reporting. If the branch is functionally useful but visually hard to distinguish from base, say that instead of calling it a polish win.
- Report both hot-path impact and lazy feature cost. If a high-quality but heavy library is justified, name the next optimization concretely, e.g. fine-grained language/theme bundles.

See `references/authenticated-nextjs-comparison.md` for the exact local Next.js/Lighthouse/Playwright pattern, including authenticated cookie headers and screenshot workflow.
...
5. **Visual/UX Evidence** - For A/B tasks, include screenshots or a side-by-side composite and a blunt tradeoff call.
