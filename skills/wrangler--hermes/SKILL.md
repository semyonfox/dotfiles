---
name: wrangler
description: "Use when cloudflare Workers CLI for deploying, developing, and managing Workers, KV, R2, D1, Vectorize, Hyperdrive, Workers AI, Containers, Queues, Workflows, Pipelines, and Secrets Store. Load before running wrangler commands to ensure correct syntax and best practices. Biases towards retrieval from Cloudflare docs over pre-trained knowledge."
...
`wrangler r2 object get` and `wrangler r2 object put` may default to local storage. For real Cloudflare R2 during migrations/backups, pass `--remote` explicitly:

```bash
wrangler r2 object get my-bucket/path/to/object --remote --file ./object
wrangler r2 object put my-bucket/path/to/object --remote --file ./object
```

When running long R2 migrations, avoid changing Wrangler login mid-run: subprocesses shelling out to Wrangler will target whichever account is currently authenticated. For cross-account backups, prefer account-stable R2 S3 credentials for the source account, then use Wrangler/API credentials for the destination account after backup verification.
...
### Migration and exact-key pitfalls

- For real Cloudflare R2 object operations, pass `--remote`; otherwise Wrangler can operate against local dev storage and report misleading "key does not exist" errors.
- For bulk migrations where exact keys matter, prefer S3-compatible API scripts over many `wrangler r2 object get/put` calls. After upload, diff the original manifest against destination remote keys, object count, and total bytes.
- Be careful with object keys containing percent-encoded sequences (`%2C`, `%26`, `%E2%80%93`, etc.). Some CLI paths may decode them into literal punctuation/Unicode and create duplicate-but-wrong keys. Verify exact key sets before deleting the source bucket.
- Test the endpoint actually used by the app. A jurisdiction endpoint such as `https://<account>.eu.r2.cloudflarestorage.com` may return `NoSuchBucket` for a bucket that works on `https://<account>.r2.cloudflarestorage.com`.
...
```bash
# Upload object
wrangler r2 object put my-bucket/path/file.txt --file ./local-file.txt --remote

# Download object
wrangler r2 object get my-bucket/path/file.txt --remote --file ./local-file.txt

# Delete object
wrangler r2 object delete my-bucket/path/file.txt --remote
```

**Pitfall:** object commands can default to local R2 storage. For production/account migration backups, pass `--remote` explicitly or use the S3-compatible API with R2 S3 credentials for bulk copies. If the destination account returns `Please enable R2 through the Cloudflare Dashboard. [code: 10042]`, R2 must be enabled in the dashboard before bucket creation/upload will work.
...
### HTTP pull consumer token pitfall

For HTTP pull consumers, do not stop at `wrangler queues list` or `GET /queues/{id}` verification. A token can list queues but still fail actual consumption. Verify the exact runtime endpoint with a harmless pull against an empty queue:

```bash
curl "https://api.cloudflare.com/client/v4/accounts/$CF_ACCOUNT_ID/queues/$QUEUE_ID/messages/pull" \
  -H "Authorization: Bearer $CLOUDFLARE_QUEUES_API_TOKEN" \
  -H "Content-Type: application/json" \
  --data '{"visibility_timeout":10000,"batch_size":1}'
```

If this returns `401 Authentication error`, regenerate the token with **Account → Queues → Edit** on the destination account. HTTP pull needs read and write because pull/ack mutate queue state.
