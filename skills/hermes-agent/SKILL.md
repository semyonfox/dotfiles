...
hermes dashboard            Launch the web dashboard
                             See references/dashboard-auth.md for LAN/systemd and login details
...
For persistent no-approval sessions across new launches, set both `approvals.mode=off` and `HERMES_YOLO_MODE=1`, then restart the CLI/gateway so the running process reloads its startup environment. See `references/yolo-mode.md` for the short checklist.
...
#### Worker execution, handoffs, and recovery

This applies to dispatcher-spawned workers as well as operators reviewing
worker output. The base lifecycle is orient → work → heartbeat →
complete/block; use the specialized `kanban_*` tools rather than assuming
the CLI exists inside a remote execution backend.

- Start with `kanban_show`: dispatch-to-start races can leave a task
  blocked, reassigned, or archived. Read prior runs and comments before
  repeating a failed approach. A timeout calls for smaller chunks; a
  spawn/config failure should be surfaced as a precise block, not retried
  blindly.
- Respect the workspace type. `scratch` is disposable; `dir:<path>` is
  persistent shared state; `worktree` is a Git workspace and should be
  committed. Do not modify outside `$HERMES_KANBAN_WORKSPACE` unless the
  task explicitly permits it.
- Make handoffs machine-usable. A completion summary should state the
  result; metadata should carry changed files, tests, decisions, sources,
  findings, or other structured evidence. Only pass `created_cards` IDs
  returned by successful `kanban_create` calls—never infer or invent IDs.
- For consequential code changes, leave the structured handoff as a
  comment and block with `review-required: …` until human review. Reserve
  completion for genuinely terminal work such as a small docs correction
  or a research deliverable.
- A block reason is a concise, answerable decision; put the full context
  in a comment. Never use `clarify` from a headless worker: it has no
  interactive user. Use `kanban_comment` + `kanban_block`.
- Heartbeats should quantify real progress and be sparse (usually every
  few minutes for long work); omit them for short tasks. Tenant-scoped
  persistent memory must be prefixed with the tenant to avoid leaks.
...
These are common Hermes-specific workflows that used to live in narrower skills and now sit here as umbrella guidance.

### Dashboard / web UI ops

When the user asks about the Hermes dashboard or web UI:

- inspect the running process and the bind address before changing anything
- distinguish the backend on 9119 from the Vite dev server and from the built bundle
- prefer localhost or SSH tunnel unless LAN exposure is intentional
- after changing binds or auth, restart the process and verify the listening socket plus a real browser or second-host check

### In-repo skill authoring

When you are writing a skill that should ship in the repo rather than under `~/.hermes/skills/`:

- write the file into the repo's `skills/<category>/<name>/SKILL.md`
- keep frontmatter valid and start at byte 0 with `---`
- match nearby peer skills for structure, tone, and level of detail
- put reusable extras in `references/`, `templates/`, or `scripts/` rather than flattening everything into one markdown file

### CLI discovery before reliance

For any external command-line tool, verify readiness before you depend on it:

1. confirm the binary exists with `command -v` or `which`
2. check the version
3. run the relevant status/auth/health probe
4. only then proceed with the task-specific operation

This avoids conflating "installed" with "ready".

### Model selection and memory curation

When selecting a model/provider for Hermes or Codex-style agent work, check local availability and usage/quota evidence before recommending a model. Match the model to task complexity: cheaper/fast models for simple routing and broad searches; stronger reasoning/coding models for multi-step repo work, debugging, or architecture decisions. Prior detailed notes are preserved in `references/llm-model-selection.md`.

When the user asks to remember, ingest, update, or clean profile context, treat durable memory as long-lived declarative facts, not task progress. Prefer user preferences and stable environment/project conventions; avoid stale artifacts such as PR numbers, completed tasks, or temporary TODOs. Prior detailed notes are preserved in `references/memory-curation.md`.

### Cron and scheduled-job operations

When creating, debugging, or modifying Hermes cron jobs, inspect existing state first and never guess job IDs. Match the job design to cron delivery semantics: `no_agent=True` delivers non-empty stdout verbatim, stays silent on empty stdout, and alerts on non-zero exit. For watchdogs and temporary release/result windows, prefer script wrappers that gate by local time, stay quiet while healthy, and self-pause after success or expiry.

For long-running script-backed jobs, the cron script timeout is controlled globally by `cron.script_timeout_seconds` in `~/.hermes/config.yaml` (default 120s in older configs). Set it with `hermes config set cron.script_timeout_seconds <seconds>` before using build/deploy scripts that can run for minutes; `_run_job_script()` reloads config when resolving the timeout, so this does not usually need a gateway restart. When a no-agent script times out, check for orphaned child processes (Gradle/Java/node/etc.) before re-running, because the parent shell may be killed while children continue.

For non-shell scripts, remember that scheduler interpreter selection may follow the file extension; wrap JavaScript/TypeScript or other runtimes in a `.sh` wrapper and point the cron job at the wrapper. Verify scripts manually before updating the cron job, then re-list or trigger the job as appropriate. Detailed recipes copied from the old cron skill are in `references/script-backed-watch-windows.md` and `references/authenticated-results-portal-watchers.md`.

### Gateway and messaging-platform operations

For Telegram/Discord/Slack/WhatsApp gateway work, separate platform permissions/intents, Hermes allowlist/routing/thread policy, and agent/session behavior. Confirm active profile, config path, gateway status, and the exact channel/thread surface before changing settings; after config changes restart the gateway and verify with a real platform message.

For Discord, distinguish free-response lobby channels from specialist channels that should require mention and auto-thread work. Keep `group_sessions_per_user: true` unless a shared room brain is intentional. Thread behavior, parent-channel inheritance, and permission pitfalls from the old gateway skill are preserved in `references/discord-thread-routing.md`.

### Agent execution-environment debugging

When a CLI tool is missing or an agent appears sandboxed, first capture the actual non-interactive environment: shell, PATH, uid, hostname, `ps -p 1`, cgroups, container markers, command paths, and startup-file differences (`bash -c` vs `bash -lc` vs `bash -ic`). Do not conclude a tool is absent until absolute paths and shell startup modes have been checked.

For Hermes gateway/systemd environments, inspect the user service environment and main PID. For coding-agent desktop/server bridges, check managed pid/port metadata and active logs before killing processes. Detailed prior cases are in `references/bunx-noninteractive-path.md` and `references/t3-code-ssh-disconnect-triage.md`.
